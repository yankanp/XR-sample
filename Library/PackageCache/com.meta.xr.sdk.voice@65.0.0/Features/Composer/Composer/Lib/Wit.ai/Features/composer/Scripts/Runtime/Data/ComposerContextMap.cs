 ...
 namespace Meta.WitAi.Composer.Data
{
    ...
    public class ComposerContextMap : PluggableBase<IContextMapReservedPathExtension>
    {
        ...
        internal ComposerContextMap(WitResponseNode response)
        {
            CheckForPlugins();
            if (response?.AsObject.HasChild(WitComposerConstants.ENDPOINT_COMPOSER_PARAM_CONTEXT_MAP) ?? false)
            {
                Data = response[WitComposerConstants.ENDPOINT_COMPOSER_PARAM_CONTEXT_MAP].AsObject;
            }
        }
